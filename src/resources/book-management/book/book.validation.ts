...
class BookVSchema {
  public create = z.object({
    ...
    genre: z
      .string({
        required_error: 'genre of the book is required',
      })
      .refine((objId) => isValidObjectId(objId), 'genre Id is not a valid id')
      .transform(getObjectIdFromString),
    /**
     *
     */
    ISBN: z
  ...
  // .refine((data) => {
  //   return data.thumbnail > data.images.length;
  // }, `thumnail index should not be greater than images length`);
  /**
   *
   */
